...
enum EPSDecoder {
    ...
    static func decode(data: Data) throws -> EPSDecodedImage {
        ...
        // Illustrator previews cover the whole artboard, so trim empty margins.
        if let r = tiffRange, let img = TIFFPreview.decode(bytes, range: r), let rgb = renderRGB(img) {
            return EPSDecodedImage(image: trimMargins(rgb), method: "tiff-preview", isPhoto: false)
        }
        // 3. XMP thumbnail.
        if let img = XMPThumbnail.decode(bytes, ps: psRange), let rgb = renderRGB(img) {
            return EPSDecodedImage(image: trimMargins(rgb), method: "xmp-thumbnail", isPhoto: true)
        }
    ...
    /// Crops away white margins around the artwork (plus a thin frame line at
    /// the very edge, which Illustrator previews often have), keeping a small
    /// padding. Returns the image unchanged if it is blank or nothing to trim.
    static func trimMargins(_ image: CGImage) -> CGImage {
        let w = image.width, h = image.height
        guard w > 16, h > 16, let space = CGColorSpace(name: CGColorSpace.sRGB),
        ...
        ctx.fill(CGRect(x: 0, y: 0, width: w, height: h))
        ctx.draw(image, in: CGRect(x: 0, y: 0, width: w, height: h))
        guard let raw = ctx.data else { return image }
        let p = raw.bindMemory(to: UInt8.self, capacity: w * h * 4)   // row 0 = top

        @inline(__always) func ink(_ x: Int, _ y: Int) -> Bool {
            let o = (y * w + x) * 4
            return p[o] < 250 || p[o + 1] < 250 || p[o + 2] < 250
        }
        func rowCoverage(_ y: Int) -> Double {
            var n = 0
            for x in 0..<w where ink(x, y) { n += 1 }
            return Double(n) / Double(w)
        }
        func colCoverage(_ x: Int) -> Double {
            var n = 0
            for y in 0..<h where ink(x, y) { n += 1 }
            return Double(n) / Double(h)
        }

        // Skip frame lines (rows/columns almost fully inked) within 4 px of each edge.
        var top = 0, bottom = h, left = 0, right = w
        for k in 0..<4 where rowCoverage(k) > 0.9 { top = k + 1 }
        for k in 0..<4 where rowCoverage(h - 1 - k) > 0.9 { bottom = h - 1 - k }
        for k in 0..<4 where colCoverage(k) > 0.9 { left = k + 1 }
        for k in 0..<4 where colCoverage(w - 1 - k) > 0.9 { right = w - 1 - k }

        var minX = Int.max, minY = Int.max, maxX = -1, maxY = -1
        for y in top..<bottom {
            for x in left..<right where ink(x, y) {
                if x < minX { minX = x }
                if x > maxX { maxX = x }
                if y < minY { minY = y }
                if y > maxY { maxY = y }
        ...
        let pad = max(4, max(w, h) * 2 / 100)
        let x0 = max(0, minX - pad), y0 = max(0, minY - pad)
        let x1 = min(w, maxX + 1 + pad), y1 = min(h, maxY + 1 + pad)
        if x0 == 0 && y0 == 0 && x1 == w && y1 == h { return image }
        guard let cropped = ctx.makeImage()?.cropping(to: CGRect(x: x0, y: y0, width: x1 - x0, height: y1 - y0))
        else { return image }
        return cropped
    }
